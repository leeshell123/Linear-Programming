function x_best = branchcut(c,A,b,lb,ub)
%UNTITLED Summary of this function goes here
%   branch cut method, add one cutting plane at each iteration
...
while i==0 || isinf(intBest) || (~isempty(nodeOrder) &&  ((intBest-min(prtOpt(nodeOrder)))/abs(intBest+min(prtOpt(nodeOrder))) > stopTol) &&  i<maxNodes)
    ...
    if i==0 || prtOpt(nodeOrder(1))<intBest
        ...
        if cutnum<=10
            ...
            [x,this_f,flag,newA,newb]=intlinprog(c,A,b,[],[],s,d,'CP',0);
        end
        ...
        if flag~=-8 && flag~=1
            ...
            if i==1
                error('BCPROG: LP infeasible or unbounded. Try another LP solver.')
            end
            f(i) = inf;
